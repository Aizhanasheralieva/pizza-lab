...
import {IPizzaDishesForm, IPizzaDishesFromAPI} from "../../../types";
...
export const fetchOnePizzaDishById = createAsyncThunk<IPizzaDishesForm, string>(
  ...
  async (id: string) => {
    const response = await axiosAPI.get<IPizzaDishesForm>(
      `variousPizzaDishes/${id}.json`,
    );
    return response.data || null;
  },
);

export const fetchAllPizzaDishes = createAsyncThunk<
  IPizzaDishesFromAPI[],
  void
>("variousPizzaDishes/fetchAllPizzaDishes", async () => {
  const response: { data: IPizzaDishesFromAPI } =
    await axiosAPI<IPizzaDishesFromAPI | null>("variousPizzaDishes.json");
...
export const updatePizza = createAsyncThunk<
  void,
  { pizzaDishId: string; pizza: IPizzaDishesFromAPI }
>("variousPizzaDishes/updatePizzaDishById", async ({ pizzaDishId, pizza }) => {
  const response = await axiosAPI.put(
    `variousPizzaDishes/${pizzaDishId}.json`,
    pizza,
  );
